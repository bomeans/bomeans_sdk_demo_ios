/*
 ============================================================================
 Name        : SETTOPRemoterViewController.m
 Version     : 1.0.0
...
#import "SETTOPChannelNumberView.h"
#import "SETTOPOtherButtonsView.h"
#import "SETTOPRemoterMatchViewController.h"
#import "UIUtil.h"
...
@interface SETTOPRemoterViewController()
{
    ...
    IBOutlet UIButton* __weak _epgButton;
    IBOutlet UILabel* __weak _titleLabel;
    IBOutlet UIButton* __weak _irStateButton;
    
    SETTOPChannelNumberView* _numberView;
    
    NSMutableArray* _keyArray;
    
    SETTOPOtherButtonsView* _otherButtonsView;
...
@implementation SETTOPRemoterViewController
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self)
    {
        ...
        _numberView = [[[NSBundle mainBundle] loadNibNamed:@"SETTOPChannelNumberView" owner:nil options:nil] objectAtIndex:0];
        _otherButtonsView = [[[NSBundle mainBundle] loadNibNamed:@"SETTOPOtherButtonsView" owner:nil options:nil] objectAtIndex:0];
...
- (void)viewDidLoad
{
    ...
    //未匹配前取得中華電信機頂盒所有key來使用
    NSArray* modelArray = [[DeviceManager sharedInstance] getDeviceModelList:self.device.typeID brandID:self.device.brandID];
    BIRModelItem* settopmodel = [modelArray objectAtIndex:0];
    id<BIRRemote> settopRemote;
    settopRemote = [[DeviceManager sharedInstance] createRemoter:self.device.typeID brandID:self.device.brandID module:settopmodel.model];
    self.device.modelID = settopmodel.model;
    self.device.remoter = settopRemote;
    self.device.keys = [settopRemote getAllKeys];
    ...
    [_keyArray addObject:[NSDictionary dictionaryWithObjectsAndKeys:@"IR_KEY_EPG", @"key", _epgButton, @"value", nil]];
...
- (IBAction)matchButtonClicked:(id)sender
{
    ...
    ^{
         ...
         ^{
            ...
            if (array.count > 0)
            {
                SETTOPRemoterMatchViewController* viewController = [[SETTOPRemoterMatchViewController alloc] init];
